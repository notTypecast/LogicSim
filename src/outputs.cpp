...
namespace logicsim
{
    namespace output
    {
        ...
        bool _5in_7SegmentDisplay::_evaluate(unsigned int out)
        {
            ...
            {
            case 1:
                return !(x[1] && x[2] && x[3] || x[1] && x[2] && !x[3] && !x[4] || !x[1] && !x[2] && x[3] && !x[4]);
            case 2:
                return !(x[2] && x[3] && x[4] || !x[1] && x[2] && !x[3] && !x[4] || !x[1] && !x[2] && !x[3] && x[4] || x[1] && !x[2] && x[3] && !x[4]);
            case 3:
                return !(!x[1] && x[4] || !x[1] && x[2] && !x[3] && !x[4] || x[1] && !x[2] && !x[3] && x[4]);
            case 4:
                return !(!x[1] && !x[2] && !x[3] || !x[1] && x[2] && x[3] && x[4] || x[1] && x[2] && !x[3] && !x[4]);
            case 5:
                return !(!x[1] && !x[2] && x[3] || !x[1] && x[3] && x[4] || !x[1] && !x[2] && !x[3] && x[4] || x[1] && x[2] && !x[3] && x[4]);
            case 6:
                return !(!x[1] && !x[2] && !x[3] && x[4] || !x[1] && x[2] && !x[3] && !x[4] || !x[1] && x[2] && x[3] && x[4] || x[1] && x[2] && !x[3] && x[4] || x[1] && !x[2] && x[3] && x[4]);
            case 7:
                return !(x[2] && x[3] && !x[4] || x[1] && x[2] && x[3] || x[1] && x[3] && x[4] || x[1] && x[2] && !x[4] || !x[1] && x[2] && !x[3] && x[4]);
            case 0:
            default:
                return x[0];
            }
