    ...
    namespace gui
    {
        ...
        void Clipboard::copy(std::vector<ComponentLabel *> components)
        {
            ...
            for (size_t i = 0; i < components.size(); ++i)
            {
                ...
                _component_reprs[i].inputs = std::vector<std::pair<ComponentRepr *, int>>(input_wires.size());
                for (size_t j = 0; j < input_wires.size(); ++j)
                {
                    if (input_wires[j] != nullptr)
                    {
                        ComponentLabel *opposite = input_wires[j]->oppositeComponent(components[i]);
                        if (component_map.find(opposite) != component_map.end())
                        {
                            _component_reprs[i].inputs[j] = {component_map[opposite], input_wires[j]->outputComponentInfo().second};
                            continue;
                        }
                    }
        ...
        void Clipboard::paste()
        {
            ...
            for (size_t i = 0; i < _component_reprs.size(); ++i)
            {
                ...
                InsertComponentCommand *insert_component = new InsertComponentCommand(_design_area, components[i]);
            ...
            for (size_t i = 0; i < _component_reprs.size(); ++i)
            {
                for (size_t j = 0; j < _component_reprs[i].inputs.size(); ++j)
                {
                    if (_component_reprs[i].inputs[j].first != nullptr)
                    {
                        ...
                        InsertWireCommand *insert_wire = new InsertWireCommand(_design_area, wire);
            ...
            emit _design_area->newUndoActionPerformed(false, _design_area->_undo_stack->canUndo(), _design_area->_undo_stack->canRedo());
