...
namespace logicsim
{
    namespace gui
    {
        DocWindow::DocWindow(QWidget *parent) : QMainWindow{parent}, _ui(new Ui::DocWindow)
        {
            _ui->setupUi(this);
            ...
            QObject::connect(_ui->browseView, SIGNAL (clicked(const QModelIndex &)), this, SLOT (_displayFileContents(const QModelIndex &)));
        ...
        void DocWindow::_populateModel(QStandardItem *parent_item, const QString &path) {
            ...
            for (const QFileInfo &entry : entries) {
                if (entry.isDir()) {
                    ...
                    QStandardItem *folder_item = new QStandardItem(title.mid(title.indexOf(" ") + 1));
                    ...
                    QStandardItem *file_item = new QStandardItem(title.mid(title.indexOf(" ") + 1));
