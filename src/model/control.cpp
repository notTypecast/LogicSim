...
namespace logicsim
{
    namespace model
    {
        ...
        {
            // Multiplexer
            Multiplexer::Multiplexer(unsigned int bits) : NInputComponent(bits + std::pow(2, bits) + 1, 5, 1),
                _bits(bits),
                _S(std::vector<Component **>(bits)), _S_out(std::vector<unsigned int *>(bits)),
                _D(std::vector<Component **>(n_inputs() - bits - 1)), _D_out(std::vector<unsigned int *>(n_inputs() - bits - 1))
            {
                _E = &_inputs[0];
            ...
            // Decoder
            Decoder::Decoder(unsigned int bits) : NInputComponent(bits + 1, 5, std::pow(2, bits)),
                _bits(bits), _outputs(std::pow(2, _bits)),
                _B(std::vector<Component **>(_bits)), _B_out(std::vector<unsigned int *>(_bits))
            {
